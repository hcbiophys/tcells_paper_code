...
#N = 10
#max_mexhs = np.linspace(12/2, 12, N)
#max_gaus1s =np.linspace(27/2, 27, N)


entropy_matrix = np.zeros(shape = (len(max_mexhs), len(max_gaus1s)))

for idx_max_mexh, max_mexh in enumerate(max_mexhs):
    for idx_max_gaus1, max_gaus1 in enumerate(max_gaus1s):
        entropies_combo = []
        for idx_repeat in range(5):
            #max_mexh = 9
            #max_gaus1 = 0.75*27
            mexh_scales = [0.5*i for i in np.linspace(2, max_mexh, 6)]
            gaus1_scales = [0.4*i for i in np.linspace(2, max_gaus1, 6)]
            ...
            class CWT():
                ...
                def transition_matrix(self, s, b, grid, stop_run_over_all = None, plot = True):
                    ...
                    if grid:
                        ...
                        colors = ['black', 'yellow', 'green', 'red', 'blue', 'orange', 'violet', 'grey', 'aqua', 'maroon', 'white']
                    ...
                    entropies_combo.append(entropy_save)
            ...
            #show_cell_series_clustered(codes = ['zm_3_6_0a-158'])
            ...
            cwt.kde(load_or_save_or_run = load_or_save_or_run, filename = filename, plot = False)
            ...
            cwt.transition_matrix(s = None, b = None, grid = True, plot = False)
            ...
            #plt.show()
        print('entropies_combo', entropies_combo)
        entropy_mean = np.mean(entropies_combo)
        entropy_matrix[idx_max_mexh, idx_max_gaus1] = entropy_mean
...
fig = plt.figure()
ax = fig.add_subplot(111)
ax.imshow(entropy_matrix, extent=[0, 1, 0, 1], cmap = 'Greens')




x_label_list = max_gaus1s
y_label_list = max_mexhs

ax.set_xticks(np.linspace(0, 1, len(max_gaus1s), endpoint = False))
ax.set_yticks([1- i for i in np.linspace(0, 1, len(max_mexhs), endpoint = False)])
