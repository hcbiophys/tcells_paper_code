...
if __name__ == '__main__':
    ...
    idx_cells_orig =  ['2_{}'.format(i) for i in range(10)] + ['3_1_{}'.format(i) for i in range(6)] + ['zm_3_3_{}'.format(i) for i in range(8)] + \
    ...
    for idx_cell in idx_cells_orig:
        #RUN WITH THIS CELL AND CHANGE THE RUN ERROR NOT TO BE DELTA_UC RELATED, BUT RATHER ... RANDOMLY PERTURB THE UROPOD in area of delta_uropod AND CALCULATE THE DIFFERENCE..?

        cells = Cells(stack_attributes_2 + stack_attributes_3, cells_model = [idx_cell], max_l = 15, keep_every_random = 1)



        cells._set_centroid_attributes('run', time_either_side = 7)
        lymphs = cells.cells[idx_cell]
        times = [lymph.t_res*lymph.frame for lymph in lymphs]
        runs = [lymph.run_uropod if lymph.run_uropod  is not None else np.nan for lymph in lymphs]
        runs_sum = np.nancumsum(runs)
        p = ax1.plot(times, runs_sum)
        runs = [lymph.run_centroid if lymph.run_centroid  is not None else np.nan for lymph in lymphs]
        runs_sum = np.nancumsum(runs)
        ax2.plot(times, runs_sum, c = p[0].get_color())
        ...
        #cells.curvatures(plot = False)
        """
        cells._set_centroid_attributes(attribute = 'run', time_either_side = 30)
        ...
        all = []
        for lymph in lymphs:
            if lymph.run_vec_theta is not None:
                if lymph.run_vec_theta <70:
                    vec1 = lymph.delta_uropod
                    vec2 = lymph.centroid - lymph.uropod
                    cos_angle = np.dot(vec1, vec2)/(np.linalg.norm(vec1)*np.linalg.norm(vec2))
                    UC_uropod_angle = (360/6.283)*np.arccos(cos_angle)
                    all.append(UC_uropod_angle)
        ...
        #cells.plot_orig_series(idx_cell=idx_cell, uropod_align = False, color_by = None, plot_every = 7)
